...
//    [body appendFormat:@"%@\r\n",upLoadUser];
    ...
    if (conn)
    {
        ...
        NSLog(@"%@",result);
        [[NSNotificationCenter defaultCenter] postNotificationName:@"uploadDone" object:@"success"];
    }
    else
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"uploadDone" object:@"fail"];
    }
...
- (void)uploadIcon
{
    ...
    //建立请求
    NSMutableURLRequest* request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:userIconUploadServerAddress]
                                                           cachePolicy:NSURLRequestReloadIgnoringLocalCacheData
                                                       timeoutInterval:10];
    //设置HTTPHeader
    [request setValue:content forHTTPHeaderField:@"Content-Type"];
    ...
    if (conn)
    {
        NSString *result = [[NSString alloc] initWithData:conn encoding:NSUTF8StringEncoding];
        NSLog(@"%@",result);
        [[NSNotificationCenter defaultCenter] postNotificationName:@"iconUploadDone" object:@"success"];
    }
    else
    {
